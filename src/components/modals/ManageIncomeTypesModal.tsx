...
interface ManageIncomeTypesModalProps {
  isOpen: boolean;
  onClose: () => void;
  onUpdate: () => void;
}

const ManageIncomeTypesModal: React.FC<ManageIncomeTypesModalProps> = ({
  isOpen,
  onClose,
  onUpdate
}) => {
  ...
  const loadIncomeTypes = async () => {
    try {
      ...
      alert('Failed to load income types');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAddType = async () => {
    if (!newType.name.trim()) {
      alert('Please enter a name for the income type');
      return;
    }
    ...
    try {
      ...
      if (error) {
        if (error.code === '23505') {
          alert('An income type with this name already exists');
        } else {
          throw error;
        }
        return;
  ...
  const handleEditType = async (id: string) => {
    ...
    try {
      ...
      alert('Failed to update income type');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDeleteType = async (id: string, name: string, isDefault: boolean) => {
    if (isDefault) {
      alert('Cannot delete default income types');
      return;
    }
  ...
  const startEdit = (incomeType: IncomeType) => {
      ...
      <div className="bg-white rounded-xl w-full max-w-2xl max-h-[90vh] overflow-hidden">
        <div className="p-6 border-b border-secondary-200">
          <div className="flex items-center justify-between">
            <h2 className="text-xl font-bold text-secondary-800">Manage Income Types</h2>
            <button
              onClick={onClose}
        ...
        <div className="p-6 max-h-[70vh] overflow-y-auto">
          ...
          <div className="mb-6 p-4 bg-primary-50 border border-primary-200 rounded-lg">
            <h3 className="text-lg font-semibold text-primary-800 mb-3">Add New Income Type</h3>
          ...
          <div className="space-y-3">
            <h3 className="text-lg font-semibold text-secondary-800">Existing Income Types</h3>
            {loading ? (
              ...
              incomeTypes.map((incomeType) => (
                <div
                  ...
                  className={`p-4 border rounded-lg ${
                    incomeType.is_default 
                      ? 'bg-secondary-50 border-secondary-200' 
                      : 'bg-white border-secondary-200'
                  }`}
                >
                  {editingId === incomeType.id ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <input
                          type="text"
                      ...
                      <div className="md:col-span-2 flex space-x-2">
                        <button
                          onClick={() => handleEditType(incomeType.id)}
                          disabled={loading}
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex-1">
                        <div className="flex items-center space-x-2">
                          <h4 className="font-semibold text-secondary-800">{incomeType.name}</h4>
                          {incomeType.is_default && (
                        ...
                        {incomeType.description && (
                          <p className="text-sm text-secondary-600 mt-1">{incomeType.description}</p>
                        )}
                      </div>
                      <div className="flex space-x-2">
                        <button
                          onClick={() => startEdit(incomeType)}
                          className="p-2 hover:bg-secondary-100 rounded-lg transition-colors"
                        >
                        ...
                        {!incomeType.is_default && (
                          <button
                            onClick={() => handleDeleteType(incomeType.id, incomeType.name, incomeType.is_default)}
...
export default ManageIncomeTypesModal;
